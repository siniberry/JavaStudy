...
○ 인터페이스(Interface)란
	...
	몸체(정의부)가 없기 떄문에 사실상 실행 부분이 존재하지 않는다.
...
○ 특징
   ...
   - 인터페이스를 implements 한 클래스는
     인터페이스의 모든 메소드를 Overridng 해야 한다.
...
public class Test119
{
	...
	//  포유류           곰
		...
		//--==>> 3.141592
		// PI 변수가 static 변수라 가능

		System.out.println(Demo.a);			//-- static 이기 때문에
		//--==>> 10
		// 일반변수처럼 보이나 인터페이스 안에 존재하는 변수이므로 자동으로 static fianl이 붙어서 마찬가지로 가능
	
		// Demo.a = 30;						//-- final 이기 때문에
		//--==>> 에러 발생 (final 이기때문에 상수화되어 변화x)		
	}
}